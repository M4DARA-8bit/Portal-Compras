...
export function normalizarPerfil(id, dados = {}) {
  ...
  return {
    ...
    // Só é usado quando o papel de Tarefas é "administrador" — restringe
    // esse admin a só ver/gerenciar as áreas listadas aqui. Vazio/ausente
    // = administra tudo (comportamento de sempre, sem restrição nenhuma).
    // Mantenha esse trecho igual ao do auth-local.js do Tarefas.
    areasGerenciadas: Array.isArray(dados.areasGerenciadas) ? dados.areasGerenciadas : [],
    localAuth: true,
    firebaseAuth: false
  };
}
